...
def next_edge_to_remove(gadget_index, general_data, gate_combinations, removed_gadgets_num, random_sel=False):
    ...
    for qubit0,qubit1 in edge_options:
        if gadget_data[qubit1,gadget_index] == 0b00: #swap needed
            edge_gate.append((apply_I, apply_I, True))
        else:
            edge_gate.append((apply_I, apply_I, False))


    if num_gadgets - removed_gadgets_num == 1: # Only one gadget left, choose any
        return edge_options[0], edge_gate[0]

    score = np.zeros((len(edge_options)), dtype=int)
    score_remove_I = np.zeros((len(edge_options)), dtype=int)
    for gadget in range(num_gadgets): # For each non-removed gadget, check how different edge+gate combinations would affect it
        ...
        for e in range(len(edge_options)):
            ...
            if qubit0_degree > 1 and qubit1_degree > 1:
                if gadget_data[qubit0,gadget] == 0b00 and gadget_data[qubit1,gadget] != 0b00 and gates0[2] == False:
                    I_score_change = 1
                elif gadget_data[qubit0,gadget] != 0b00 and gadget_data[qubit1,gadget] == 0b00 and gates0[2] == True:
                    I_score_change = 1
                elif gadget_data[qubit0,gadget] != 0b00 and gadget_data[qubit1,gadget] == 0b00:
                    I_score_change = -1
            ...
            # Pair is last pair in a brach in this gadget also. Try turn first qubit to I
                ...
                if gadget_data[qubit0,gadget] != 0b00 and gadget_data[qubit1,gadget] != 0b00 and gates0[2] == False:
                    score_change = 1
                if gadget_data[qubit0,gadget] != 0b00 and gadget_data[qubit1,gadget] != 0b00 and gates0[2] == True:
                    I_score_change = -1
                elif gadget_data[qubit0,gadget] != 0b00 and gadget_data[qubit1,gadget] == 0b00 and gates0[2] == True:
                    I_score_change = 1
            ...
            # Same situation but reversed
                ...
                if gadget_data[qubit1,gadget] != 0b00 and gadget_data[qubit0,gadget] != 0b00 and gates0[2] == True:
                    score_change = 1
                if gadget_data[qubit1,gadget] != 0b00 and gadget_data[qubit0,gadget] != 0b00 and gates0[2] == False:
                    I_score_change = -1
                elif gadget_data[qubit1,gadget] != 0b00 and gadget_data[qubit0,gadget] == 0b00 and gates0[2] == False:
                    I_score_change = 1

            # Pair touches a branch. Try not to extend branch
            elif qubit0_degree == 0 and qubit1_degree > 0 and gates0[2] == False:
                score_change = -1
            elif qubit1_degree == 0 and qubit0_degree > 0 and gates0[2] == True:
                score_change = -1
            ...
            score_remove_I[e] += I_score_change
            score[e] += score_change

    option_possibilities = []
    for e in range(len(edge_options)):
        gates = edge_gate[e]
        option_possibilities.append((e, gates, score[e], score_remove_I[e]))
    option_possibilities.sort(key=lambda x: (-x[2], -x[3]))
#    print(option_possibilities)
    if random_sel:
...
#    print(edge_gates)
    return edge_options[edge_gates[0]], edge_gates[1]
    ...
    for i in range(num_gadgets):
        ...
        for q in range(num_qubits):
            if gadget_data[q,i] != 0b00:
                h_node = q
        order.append((i, s_nodes, nodes+s_nodes, h_node, gadget_rand_num[i]))
#        order.append((i, s_nodes, nodes+s_nodes, 1, gadget_rand_num[i]))
    if random_sel:
        order.sort(key=lambda x: (x[2], x[1], x[3], x[4]))
    else:
    ...
    gs = []
    for g in order:
        if g[1] == g0[1] and g[2] == g0[2]:
            c += 1
            gs.append(g[0])
#    if c > 1:
#        print('THERE ARE CHOICES:', c, gs)
#        input()
    return order[0][0]
        ...
        for j, first_qubit in enumerate([apply_I, apply_vdg, apply_sdg, apply_svs, apply_sv, apply_vs]):
            for k, second_qubit in enumerate([apply_I, apply_vdg, apply_sdg, apply_svs, apply_sv, apply_vs]):
                if paulis_options[i,j,k] == 1:
                    options.append((first_qubit, second_qubit, cnot_reversed))

    options2 = 0 
    for i, cnot_reversed in enumerate([False, True]):
        for j, first_qubit in enumerate([apply_I, apply_vdg, apply_vs]):
            for k, second_qubit in enumerate([apply_I, apply_vdg, apply_sdg, apply_sv]):
                if (first_qubit, second_qubit, cnot_reversed) in options:
                    options2 |= 1 << (j*8 + k*2 + i)
...
def get_gate(gate_set):
    return get_gates(gate_set)[0]
    cnot_reversed = [False, True]
    first = [apply_I, apply_vdg, apply_vs]
    second = [apply_I, apply_vdg, apply_sdg, apply_sv]
    i = -1
    while gate_set > 0:
        gate_set >>= 1
        i +=1
    return first[i//4], second[i%4], False
